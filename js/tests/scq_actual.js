...
export const SCQ_Actual = new TestBase({
  name: "SCQ Forma B - Situación actual", // ultimos 3 meses
  info: "Responda pensando en el comportamiento de la persona durante los últimos 3 meses.",
  questions: [
  ...
  calculator: resp => {
    if (resp[0] === 1) {
      return `<p class="highlight">El SCQ no es válido: la persona no tiene lenguaje verbal suficiente</p>`;
    }
    const score = resp.slice(1).reduce((a, b) => a + b, 0);
    const interpretacion = score >= 15 ? "ALTA probabilidad de TEA en la situación actual" : "BAJA probabilidad actual";
    return `
      <p><strong>Puntuación SCQ Situación actual:</strong> ${score} / 39</p>
      <hr>
      <p class="highlight">${interpretacion}</p>
      <small>Útil para seguimiento o evaluación de cambio.</small>
    `;
  }
});